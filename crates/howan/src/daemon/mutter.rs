...
//!    - [`RearmKind::AfterActive`] — `DpmsHandoff`. The user is still
...
//! compositor never reaches DPMS off — the DpmsHandoff is functionally
...
//! does not apply on the post-DpmsHandoff path: by the time `RearmKind::AfterActive`
//! reaches the watch loop, the DpmsHandoff has already destroyed the `Saver` and
//! released the inhibitor, so the active watch sees unmasked state and fires
//! on real user activity. The `Inhibiting` input path still uses
...
trait IdleMonitor {
    ...
    /// gate the next idle watch on real user activity after a DpmsHandoff
    /// (see the module docs).
...
/// user-active watch first, then an idle watch once it fires" (the DpmsHandoff
/// state). See the module-level "Re-arm strategy" docs for the full
/// rationale.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum RearmKind {
    /// The user just produced input — re-add the idle watch immediately.
    Immediate,
    /// The DpmsHandoff timer dismissed the saver while the user was still idle —
...
impl IdleSource for MutterIdleSource {
    ...
    fn rearm_after_active(&self) -> Result<(), Box<dyn Error>> {
        // DpmsHandoff path; see the module "Re-arm strategy" docs.
...
/// saver's two `SaverPhase`s in `crate::app`; "step" is used here to avoid
/// the name collision.
///
...
fn run_watch_loop(
    ...
    // first arming, `dismiss` after an `Inhibiting` input dismiss, and
    // `add_user_active_watch` after a DpmsHandoff (the active-watch
    ...
    'cycles: loop {
        ...
        // saver `Inhibiting` input dismiss (immediate re-arm) or
        // `rearm_after_active` after a saver DpmsHandoff (gated on
        ...
        // idle watch. By the time we reach this point the saver DpmsHandoff
...
mod tests {
    ...
    fn rearm_after_active_before_start_is_ok() {
        // The DpmsHandoff re-arm primitive shares the same "signal-or-noop" shape
